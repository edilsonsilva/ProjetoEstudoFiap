...
const cce = require("clientes-edilson-email-cpf")
...
app.get("/admin/users",cors(optionsConfig),autentica,(req,res)=>{
    if(req.content.usuario != "veronica"){
        res.status(403).send({rs:"Você não está autorizado a ver o conteúdo desta página"})
        return
    }
    cliente.find((err,dados)=>{
        if(err){
...
app.post("/cadastro",(req,res)=>{
    ...
    dados.save().then(()=>{
        ...
        if(erro.code === 11000){
            res.status(400).send({rs:`Usuário existente. Por favor tente outro.`})    
        }
